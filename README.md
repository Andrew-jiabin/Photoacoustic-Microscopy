...
- 532 nm blocked-beam noise subtraction is prompted before acquisition by default; set `PAM_532_NOISE_PROMPT_ENABLE=0` to skip the prompt. The restoration noise waveform is saved under `metadata.noise_532.average_waveform`; details are in `reference/PAM_Nanomax_laser_integration_notes.md`.
